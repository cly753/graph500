...
enum {
    s_minimum, s_firstquartile, s_median, s_thirdquartile, s_maximum, s_mean, s_std, s_LAST
};
...
int main(int argc, char **argv) {
    ...
    if (tg.data_in_file) {
        ...
        MPI_File_open(MPI_COMM_WORLD, (char *) filename,
                      MPI_MODE_RDWR | MPI_MODE_CREATE | MPI_MODE_EXCL | MPI_MODE_DELETE_ON_CLOSE | MPI_MODE_UNIQUE_OPEN,
    ...
    double make_graph_start = MPI_Wtime();
    {
        ...
        int ranks_per_row =
                ((nglobalverts + CHAR_BIT - 1) / CHAR_BIT + bitmap_size_in_bytes - 1) / bitmap_size_in_bytes;
        ...
        if (cart_comm != MPI_COMM_NULL) {
            ...
            if (tg.data_in_file) {
                ...
                int64_t nedges =
                        FILE_CHUNKSIZE * (tg.nglobaledges / ((int64_t) FILE_CHUNKSIZE * nrows * ranks_per_row)) +
            ...
            for (block_idx = 0; block_idx < block_limit; ++block_idx) {
                ...
                if (tg.data_in_file && my_col == (block_idx % ranks_per_row)) { /* Try to spread writes among ranks */
                    MPI_File_write_at(tg.edgefile, start_edge_index, actual_buf, edge_count, packed_edge_mpi_type,
                                      MPI_STATUS_IGNORE);
                }
                ptrdiff_t i;
#ifdef _OPENMP
