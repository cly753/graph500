    ...
    while (1) {
            ...
            for (j = g.rowstarts[VERTEX_LOCAL(oldq[i])]; j < j_end; ++j) {
                ...
                if (owner == rank) {
                    ...
                    if (outgoing_counts[owner] == coalescing_size * 2) {
                        MPI_Isend(&outgoing[owner * coalescing_size * 2], coalescing_size * 2, MPI_INT64_T, owner, 0,
        ...
        for (offset = 1; offset < size; ++offset) {
            ...
            if (outgoing_counts[dest] != 0) {
                ...
                MPI_Isend(&outgoing[dest * coalescing_size * 2], outgoing_counts[dest], MPI_INT64_T, dest, 0,
            ...
            MPI_Isend(&outgoing[dest * coalescing_size * 2], 0, MPI_INT64_T, dest, 0, MPI_COMM_WORLD,
